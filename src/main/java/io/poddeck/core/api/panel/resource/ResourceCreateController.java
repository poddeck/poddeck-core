package io.poddeck.core.api.panel.resource;
...
@RestController
public final class ResourceCreateController extends ClusterRestController {
  private final AgentRegistry agentRegistry;
  private final AgentCommandFactory commandFactory;

  private ResourceCreateController(
  ...
  @PanelEndpoint
  @RequestMapping(path = "/resource/create/", method = RequestMethod.POST)
  public CompletableFuture<Map<String, Object>> createResource(
    ...
    return findCluster(request)
      .thenCompose(cluster -> createResource(cluster, raw));
  }

  private CompletableFuture<Map<String, Object>> createResource(
    ...
    return commandFactory.create(agent.get())
      .execute(ResourceCreateRequest.newBuilder().setRaw(raw).build(),
        ResourceCreateResponse.class)
