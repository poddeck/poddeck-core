...
public final class CronJobDeleteController extends CronJobRestController {
  ...
  @PanelEndpoint
  @RequestMapping(path = "/cron-job/delete/", method = RequestMethod.POST)
  public CompletableFuture<Map<String, Object>> deleteCronJob(
    ...
    return findCluster(request)
      .thenCompose(cluster -> deleteCronJob(cluster, namespace, cronJob));
  }

  private CompletableFuture<Map<String, Object>> deleteCronJob(
    ...
    return commandFactory.create(agent.get())
      .execute(CronJobDeleteRequest.newBuilder()
          .setNamespace(namespace).setCronJob(cronJob).build(),
        CronJobDeleteResponse.class)
      .thenApply(response -> processDeleteCronJobResult(cluster,
        cronJob, response));
  }

  private Map<String, Object> processDeleteCronJobResult(
    Cluster cluster, String cronJob, CronJobDeleteResponse response
  ) {
    if (response.getSuccess()) {
      notificationDispatch.dispatch(cluster.id(), NotificationType.REPORT,
        "panel.cron-job.delete.notification.title",
        "panel.cron-job.delete.notification.description", cronJob);
    }
