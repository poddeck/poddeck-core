...
public final class CronJobCreateController extends CronJobRestController {
  ...
  @PanelEndpoint
  @RequestMapping(path = "/cron-job/create/", method = RequestMethod.POST)
  public CompletableFuture<Map<String, Object>> createCronJob(
    ...
    return findCluster(request)
      .thenCompose(cluster -> createCronJob(cluster, raw));
  }

  private CompletableFuture<Map<String, Object>> createCronJob(
    ...
    return commandFactory.create(agent.get())
      .execute(CronJobCreateRequest.newBuilder().setRaw(raw).build(),
        CronJobCreateResponse.class)
      .thenApply(response -> processCreateCronJobResult(cluster, response));
  }

  private Map<String, Object> processCreateCronJobResult(
    Cluster cluster, CronJobCreateResponse response
  ) {
    if (response.getSuccess()) {
      notificationDispatch.dispatch(cluster.id(), NotificationType.REPORT,
        "panel.cron-job.create.notification.title",
        "panel.cron-job.create.notification.description", response.getCronJob());
    }
    return Map.of("success", response.getSuccess(),
      "namespace", response.getNamespace(),
      "cron_job", response.getCronJob());
  }
}
