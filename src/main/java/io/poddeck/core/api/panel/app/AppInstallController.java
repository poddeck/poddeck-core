...
public final class AppInstallController extends AppRestController {
  ...
  @PanelEndpoint
  @RequestMapping(path = "/app/install/", method = RequestMethod.POST)
  public CompletableFuture<Map<String, Object>> installApp(
    ...
    return findCluster(request)
      .thenCompose(cluster -> installApp(cluster, name, chart, namespace, version));
  }

  private CompletableFuture<Map<String, Object>> installApp(
  ...
  private Map<String, Object> processInstallAppResult(
    ...
    if (response.getSuccess()) {
      notificationDispatch.dispatch(cluster.id(), NotificationType.REPORT,
        "panel.app.install.notification.title",
        "panel.app.install.notification.description", chart);
    }
