...
@RestController
public final class AuditFindController extends AuditRestController {
  private final AgentRegistry agentRegistry;
  private final AgentCommandFactory commandFactory;

  private AuditFindController(
  ...
  @PanelEndpoint
  @RequestMapping(path = "/audit/find/", method = RequestMethod.GET)
  public CompletableFuture<Map<String, Object>> findAudit(
    HttpServletRequest request
  ) {
    return findCluster(request).thenCompose(this::findAudit);
  }

  private CompletableFuture<Map<String, Object>> findAudit(Cluster cluster) {
    ...
    return commandFactory.create(agent.get())
      .execute(AuditFindRequest.newBuilder().build(),
        AuditFindResponse.class)
      .thenApply(response -> Map.of("success", response.getSuccess(),
        "audit", assembleAuditInformation(response.getAudit())));
  }
}
