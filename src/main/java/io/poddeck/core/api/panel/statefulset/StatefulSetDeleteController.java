...
@RestController
public final class StatefulSetDeleteController extends StatefulSetRestController {
  private final AgentRegistry agentRegistry;
  private final AgentCommandFactory commandFactory;
  private final NotificationDispatch notificationDispatch;

  private StatefulSetDeleteController(
  ...
  @PanelEndpoint
  @RequestMapping(path = "/stateful-set/delete/", method = RequestMethod.POST)
  public CompletableFuture<Map<String, Object>> deleteStatefulSet(
    ...
    return findCluster(request)
      .thenCompose(cluster -> deleteStatefulSet(cluster, namespace, statefulSet));
  }

  private CompletableFuture<Map<String, Object>> deleteStatefulSet(
    ...
    return commandFactory.create(agent.get())
      .execute(StatefulSetDeleteRequest.newBuilder()
          .setNamespace(namespace).setStatefulSet(statefulSet).build(),
        StatefulSetDeleteResponse.class)
      .thenApply(response -> processDeleteStatefulSetResult(cluster,
        statefulSet, response));
  }

  private Map<String, Object> processDeleteStatefulSetResult(
    Cluster cluster, String statefulSet, StatefulSetDeleteResponse response
  ) {
    if (response.getSuccess()) {
      notificationDispatch.dispatch(cluster.id(), NotificationType.REPORT,
        "panel.stateful-set.delete.notification.title",
        "panel.stateful-set.delete.notification.description", statefulSet);
    }
