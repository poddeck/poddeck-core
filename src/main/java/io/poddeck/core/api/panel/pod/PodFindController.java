...
@RestController
public final class PodFindController extends PodRestController {
  private final AgentRegistry agentRegistry;
  private final AgentCommandFactory commandFactory;

  private PodFindController(
  ...
  @RequestMapping(path = "/pod/find/", method = RequestMethod.POST)
  public CompletableFuture<Map<String, Object>> findPod(
    ...
    return findCluster(request)
      .thenCompose(cluster -> findPod(cluster, namespace, pod));
  }

  private CompletableFuture<Map<String, Object>> findPod(
    ...
    return commandFactory.create(agent.get())
      .execute(PodFindRequest.newBuilder()
        .setNamespace(namespace).setPod(pod).build(), PodFindResponse.class)
      .thenApply(response -> Map.of("success", response.getSuccess(),
        "pod", assemblePodInformation(response.getPod())));
  }
}